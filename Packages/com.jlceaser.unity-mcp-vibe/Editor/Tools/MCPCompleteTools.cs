        ...
        public static object HandleCommand(JObject @params)
        {
            ...
            switch (action)
            {
                ...
                case "get_areas":
                    ...
                    try
                    {
                        var getAreaNamesMethod = typeof(UnityEngine.AI.NavMesh).GetMethod("GetAreaNames",
                            BindingFlags.Static | BindingFlags.Public);
                        if (getAreaNamesMethod != null)
                        {
                            string[] names = (string[])getAreaNamesMethod.Invoke(null, null);
                            for (int i = 0; i < names.Length; i++)
                            {
                                if (!string.IsNullOrEmpty(names[i]))
                                    areas.Add(new { index = i, name = names[i], cost = 1f });
                            }
                        }
                    }
                    catch { }
                    if (areas.Count == 0)
                    {
                        // Fallback for Unity versions without GetAreaNames()
                        areas.Add(new { index = 0, name = "Walkable", cost = 1f });
                        areas.Add(new { index = 1, name = "Not Walkable", cost = 1f });
                        areas.Add(new { index = 2, name = "Jump", cost = 1f });
                    }
        ...
        public static object HandleCommand(JObject @params)
        {
            ...
            switch (action)
            {
                case "create":
                    ...
                    var physMat = new PhysicsMaterialType(matName);
                ...
                case "get":
                    ...
                    var getMat = AssetDatabase.LoadAssetAtPath<PhysicsMaterialType>(getPath);
                ...
                case "set":
                    ...
                    var setMat = AssetDatabase.LoadAssetAtPath<PhysicsMaterialType>(setPath);
                ...
                case "assign":
                    ...
                    var assignMat = AssetDatabase.LoadAssetAtPath<PhysicsMaterialType>(assignPath);
                ...
                case "list":
                    ...
                    var materials = guids.Select(g => {
                        ...
                        var m = AssetDatabase.LoadAssetAtPath<PhysicsMaterialType>(p);
