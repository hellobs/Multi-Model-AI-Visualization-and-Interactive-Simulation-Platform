...
>
> 🚫 **v0.4(2026-08-30)战略调整:本仓库已冻结,不再投入开发。**
> 与仝牧共识:放弃 Unity 复现,IVD 可视化与交互仿真统一以 **provenance(Web 框架)** 为唯一载体;
> 外部平台嵌入采用 Web 方式(iframe/组件),不限实现手段。本仓库内容仅存档参考,不要新建/修改开发计划。
...
| 11 | **战略调整(重大变更)** | **放弃 Unity 复现,统一以 provenance(Web)为载体;嵌入外部平台走 Web 方式(iframe/组件),不限实现手段。本仓库冻结** | 2026-08-30 |
